-- FASE 6: Processar comandos e vendas dos novos agendamentos

-- Atualizar os novos comandos criados automaticamente
...
AND status = 'open'
AND id NOT IN (SELECT DISTINCT c.id FROM public.commands c JOIN public.sales s ON c.id IS NOT NULL WHERE c.barbershop_id = '9ccfd4a2-3bc1-41be-933b-51e94d0dc29a' AND s.barbershop_id = '9ccfd4a2-3bc1-41be-933b-51e94d0dc29a');

-- Inserir vendas para os novos comandos fechados
...
AND NOT EXISTS (
  ...
  AND s.sale_time = a.end_time
  AND s.barbershop_id = c.barbershop_id
);

-- Verificar totais finais atualizados
SELECT 

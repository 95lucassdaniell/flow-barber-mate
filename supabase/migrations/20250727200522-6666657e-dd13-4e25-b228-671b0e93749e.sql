-- Povoar comandas fechadas zeradas com dados realistas
DO $$
DECLARE
    comando RECORD;
    servico_id uuid;
    ...
    contador integer := 0;
BEGIN
    -- Processar TODAS as comandas fechadas com total_amount = 0
    FOR comando IN 
        SELECT c.id, c.barber_id, c.barbershop_id, c.client_id
        FROM commands c 
        WHERE c.total_amount = 0 
        AND c.status = 'closed'
        ORDER BY c.created_at DESC -- processar as mais recentes primeiro
    LOOP
        ...
        -- Definir método de pagamento aleatório (comandas fechadas já têm payment_method)
        -- Apenas atualizamos se não tiver
        rand_num := random();
        IF rand_num <= 0.40 THEN
        ...
        -- Atualizar comando com o valor total e garantir payment_method
        UPDATE commands 
        SET total_amount = preco_servico,
            payment_method = COALESCE(payment_method, metodo_pagamento),
            payment_status = 'paid', -- comandas fechadas devem estar pagas
            updated_at = now()
        WHERE id = comando.id;
        
        -- A cada 100 processados, dar um feedback
        IF contador % 100 = 0 THEN
            RAISE NOTICE 'Processadas % comandas fechadas...', contador;
        END IF;
        
    END LOOP;
    
    RAISE NOTICE 'Total de % comandas fechadas povoadas com sucesso!', contador;
END $$;
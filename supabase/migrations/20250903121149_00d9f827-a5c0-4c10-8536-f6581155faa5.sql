...
-- Create policy for whatsapp_scheduled_jobs if it doesn't exist
DO $$ 
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'whatsapp_scheduled_jobs' AND policyname = 'Service role can manage scheduled jobs') THEN
    EXECUTE 'CREATE POLICY "Service role can manage scheduled jobs" ON public.whatsapp_scheduled_jobs USING (auth.role() = ''service_role'')';
  END IF;
END $$;
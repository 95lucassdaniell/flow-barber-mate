-- Povoar comandas zeradas com dados realistas
DO $$
DECLARE
    comando RECORD;
    servico_id uuid;
        ...
        IF incluir_produto THEN
            ...
            IF produto_id IS NOT NULL THEN
                ...
                SELECT 
                    ...
                    p.price, p.price, 0, 0
                FROM products p WHERE p.id = produto_id;
                
                preco_servico := preco_servico + (SELECT price FROM products WHERE id = produto_id);
            END IF;
        END IF;

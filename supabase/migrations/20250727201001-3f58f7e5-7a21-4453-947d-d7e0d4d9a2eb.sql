-- Fechar todas as comandas abertas com distribuição de pagamento
DO $$
DECLARE
    comando RECORD;
...
BEGIN
    ...
    FOR comando IN 
        SELECT c.id, c.barber_id, c.barbershop_id, c.client_id, c.total_amount, c.appointment_id
        FROM commands c 
        WHERE c.status = 'open'
    ...
    LOOP
        ...
        INSERT INTO sales (
            ...
            comando.barbershop_id, comando.client_id, comando.barber_id, comando.appointment_id,
        ...
        FOR item IN 
            SELECT ci.*, s.name as service_name, p.name as product_name
        ...
        LOOP
            ...
            IF item.service_id IS NOT NULL AND item.commission_amount > 0 THEN
                INSERT INTO commissions (
                    ...
                    sale_id, (SELECT id FROM sale_items WHERE sale_id = sale_id AND service_id = item.service_id ORDER BY created_at DESC LIMIT 1),

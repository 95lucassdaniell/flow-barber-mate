-- Finalizar o particionamento - migração dos dados e configuração completa
...
-- Passo 10: Configurar RLS nas novas tabelas particionadas
...
-- Passo 11: Atualizar função de estatísticas para trabalhar com partições
CREATE OR REPLACE FUNCTION public.get_table_stats_partitioned()
RETURNS TABLE(
    table_name text,
...
BEGIN
    ...
    SELECT 
        ...
        pg_size_pretty(MAX(pg_total_relation_size('public'||'.'||tablename)))
    FROM pg_tables 
